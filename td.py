...
files_name = ['T:\poubelle_willi\preprocess-topaz-data732\\' + file_name for file_name in os.listdir('T:\poubelle_willi\preprocess-topaz-data732\\') if '.json' in file_name]
    ...
    for fr in data['metadata-all'].values():
        for year in fr["year"].items():
            for keyword in year[1]["kws"].items():
                keywords_count.append( [int(year[0]), str(keyword[0]), int(keyword[1]) ])
            
df = pd.DataFrame.from_dict(keywords_count).rename(columns = {0:"year",1:"kw",2:"amount"})
df = df.groupby('year')['kw','amount'].apply(lambda grp: grp.nlargest(10, 'amount'))
df = df.reset_index()
print(df)
